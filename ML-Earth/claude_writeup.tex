...
Traditional whitetail dental aging relies on sequential decision trees based on tooth count, premolar crest structure, and dentine-to-enamel ratios (DER) of molar teeth \cite{1949severinghaus, 1980larson}. The standard protocol is as follows:

\begin{enumerate}
    \item Count the number of teeth in the jawbone. Newborn deer will only have four teeth.
    \item If there are fewer than six teeth but more than four, the deer is a fawn (0.5 years old).
    \item If there are six teeth, count the crests on $P_3$. If three crests are present, the deer is 1.5 years old.
...
This protocol assumes wear progresses predictably, but DER reliability remains questionable, as Meares et al. \cite{2006meares} found DER could not reliably separate 2.5-4.5 year classes due to individual variation, undermining steps 5-8 of the decision tree.

\subsection{Dataset}

The jawbone image dataset comprises 243 colored images collected from 17 independent sources: Quality Deer Management Association, National Deer Association, state wildlife agencies, and university wildlife programs. Unlike trail camera images, no restrictions were placed on geographic region or sex. Images were captured from online educational materials (videos, tutorials, blogs) and processed to remove text annotations that might provide age information to the model.
