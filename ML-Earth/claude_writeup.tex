...
Attention map analysis (Figure \ref{fig:jawbone-attention-maps}) confirmed the model's ability to learn dental-specific features matching TRW criteria:
\begin{itemize}
	\item \textbf{0.5 years}: Attention is concentrated on molars, consistent with tooth count assessment
	\item \textbf{1.5 years}: Attention is focused on premolars, matching crest-counting criteria  
 	\item \textbf{2.5 years}: Attention is distributed across the molar region, corresponding to $M_1$ DER evaluation
	\item \textbf{3.5 years}: Focus targets posterior molars ($M_2$-$M_3$), consistent with sequential DER assessment
	\item \textbf{4.5 years}: Attention remains focused on the molar region
...
In each case, attention maps focused exclusively on dental features despite variable backgrounds, finger presence, and jawbone orientations, confirming the model's ability to identify and ignore artifacts, learning genuine biological aging signatures.

\section{Discussion}

Together, the trail camera and jawbone computer vision systems address distinct wildlife management needs. The ability to quickly and accurate estimate the age of whitetail bucks from tail camera imagery enables efficient non-invasive monitoring of live deer populations. Wildlife agencies and outdoor enthusiasts can estimate age structure without harvest data, supporting pre-season population assessments and real-time management decisions. The $76.7\%$ accuracy exceeds human performance and meets the $70\%$ threshold for management applications. Although the approach inherently suffers from AOTH's fundamental limitation (morphological variation due to nutrition and genetics), the model's plausability is demonstrated by its ability to identify and analyze genuine age-related features.

Alternatively, the dental assessment model provides higher accuracy ($90.7\%$ cross-validation, $77.6\%$ test) for harvested specimens. State wildlife agencies processing hundreds to thousands of jaw samples annually could deploy this system for rapid initial screening, flagging ambiguous cases  with low confidence scores for expert review. The approach dramatically reduces manual TRW analysis time while maintaining or exceeding traditional accuracy. Unlike CA, it requires no specialized laboratory processing or multi-week delays. The automated system enables consistent, repeatable analysis without inter-observer variation, can be used in the field, and naturally handles variations in background, lighting, and jawbone rotation.

In both computer vision systems, attention map analysis provides critical validation that both models learn biologically meaningful features rather than spurious correlations. For trail cameras, the focus on body morphology like the neck, chest, and stomach; antlers are ignored altogether, directly matches expert recommendations \cite{1999demarais, 2003richards}. For dental analysis, the progression from tooth count in fawns to premolar structure in yearlings to sequential molar wear in mature deer precisely follows the TRW decision tree \cite{1949severinghaus, 1980larson} despite never being explicitly programmed with these rules. This biological plausibility distinguishes our approach from pure "black box" machine learning -- the models discover and apply the same anatomical features wildlife biologists use, lending credibility to their predictions and facilitating acceptance by wildlife management professionals.

\subsection{Limitations and Future Work}

Despite the model's advantages, there are several constraints to consider. First, the size of both datasets (197 and 243 images) remains modest compared to typical deep learning applications. The scarcity of known-age deer data ultimately results in specimen-level leakage; because of this, the dental dataset likely contains multiple images from individual specimens split across training and test sets. While attention maps suggest the model learns biological features rather than specimen-specific artifacts, future work should implement specimen-level splitting to eliminate this potential issue. On the other hand, the same data scarcity reflects the practical reality of wildlife research: professionally verified data are expensive and time-consuming to obtain. Data augmentation and transfer learning mitigate this limitation, but larger datasets from institutional collections would likely improve performance. The

Although both datasets span multiple geographic regions, systematic validation across different states, habitats, and subspecies would further confirm the model's generalization. Body morphology and tooth wear patterns may vary with local nutrition, genetics, and environmental conditions.

From an analysis point of view, both models show reduced performance for 2.5-3.5 year classes, reflecting the genuine biological challenge of distinguishing deer in transitional growth phases. The same limitation also affects human experts and may represent an inherent ceiling on classification accuracy. Although deploying CV systems may conserve resources, they would also benefit from calibrated confidence scores, enabling automated flagging of ambiguous cases for expert review.

Based on these limitations, future development of similar models should prioritize collaboration with state agencies and research institutions to access larger, specimen-tracked datasets. The models would also benefit from geographic validation studies and integration with existing wildlife management databases and workflows. Once established, similar models could be extended and specialized for other cervid species where similar aging principles apply like mule deer, elk, and moose.

\subsection{Practical Implications}

These systems offer immediate practical value for many organizations. For wildlife agencies, automated dental analysis could process the backlog of jaw samples most agencies face during hunting season, providing timely age-structure data for next-year harvest regulation. Similarly, trail camera analysis enables population monitoring without harvest data. For academics and researchers, the $90.7\%$ cross-validation accuracy of the dental model well exceeds the $80\%$ threshold for research applications, particularly when combined with manual review of low-confidence cases. Standardized, repeatable analysis eliminates inter-observer variation. 

For hunters and enthusiasts, accessible trail camera analysis tools could improve selective harvest decisions, supporting quality deer management goals. Educational applications could help hunters develop better visual assessment skills and improve the overall well being of whitetail populations. For small organizations, this work demonstrates that effective computer vision tools can be developed using publicly available educational resources, potentially democratizing access to automated analysis capabilities previously available only to well-funded institutions.

\section{Conclusions}

This study presents the first comprehensive computer vision approach to white-tailed deer age estimation, demonstrating that transfer learning with CNN ensembles achieves breakthrough performance for both field (trail camera) and post-harvest (dental) scenarios. The ResNet-18 ensemble ($76.7\% \pm 5.9\%$ cross-validation accuracy) exceeds human AOTH performance and morphometric models while meeting management accuracy thresholds. The EfficientNet ensemble ($90.7\% \pm 2.6\%$ cross-validation accuracy, $77.6\%$ test accuracy) substantially outperforms traditional dental analysis in accuracy, speed, and repeatability.

Attention map validation confirms that both models learn biologically relevant features that align with expert knowledge, lending credibility to their predictions and facilitating professional acceptance. These complementary systems offer wildlife managers practical tools to reduce assessment workload while maintaining or exceeding current accuracy standards.

Furthermore, the success of transfer learning with limited datasets demonstrates broader implications. For instance, domain experts across wildlife biology can leverage pre-trained CNNs to develop specialized analysis tools without requiring massive data collection efforts or extensive machine learning expertise. As wildlife agencies and researchers adopt these approaches, automated age estimation could transform how deer populations are monitored and managed across North America.
