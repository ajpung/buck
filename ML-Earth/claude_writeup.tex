...
For live deer, wildlife professionals and hunters rely on "aging on the hoof" (AOTH), a set of guidelines to visually assess body proportions, antler characteristics, and behavior from trail camera images or field observations. First documented in 1978 \cite{1978knowlton}, AOTH attempts to predict deer age based on morphological features including chest depth, stomach sag, neck thickness, and leg proportions \cite{1996kroll, 1999demarais, 2003richards, 2008hellickson}. Despite extensive training materials, however, human accuracy remains problematic -- in an extensive and detailed study on AOTH accuracy, Gee et al. \cite{2013gee} found wildlife enthusiasts and trained professionals achieved only $36\%$ age prediction accuracy in systematic testing with individual scores ranging between $16-56\%$. In the smaller dataset collected for this study, average human prediction accuracy was found to be slightly higher, at $60.46\%$ \cite{2025pung}; in both datasets, prediction accuracy was found to decrease as the age of the buck increased. Neither group approaches the $70\%$ threshold professionals themselves identify as necessary for management decisions or the $80\%$ threshold required for research applications \cite{2013gee}.

Morphometric approaches offer marginal improvement. Flinn's complex model based on 64 body measurement ratios achieved $63\%$ accuracy during post-breeding periods \cite{2010flinn}, still inadequate for practical applications. One fundamental constraint of morphometric predictions is that body morphology varies substantially with nutrition, genetics, and environmental conditions, making visual assessment inherently unreliable \cite{2013gee}.

For harvested deer, postmortem dental analysis provides two additional aging options: tooth replacement and wear (TRW) and cementum annuli (CA). TRW examines tooth eruption patterns and wear characteristics following established criteria \cite{1949severinghaus, 1980larson}. Alternatively, CA counts growth rings in tooth cementum cross-sections formed via annual cementum deposition \cite{1949severinghaus, 1963low, 1966ransom, 1989jacobson, 2006meares, 2020adams}. Both methods require specialized expertise and suffer from inconsistencies. TRW accuracy varies with soil abrasiveness and analyst experience \cite{1966gilbert, 1967ludwig, 1979cook}, while CA faces technical challenges including indistinct, incomplete, or condensed annulus patterns \cite{1966ransom,1979cook} and processing delays, with professional laboratories requiring weeks per sample \cite{2012nda}. Studies comparing TRW and CA show mixed results: some report equivalent performance \cite{1989jacobson}, others favor CA \cite{2000hamlin, 2013cooper}, while still others demonstrate TRW superiority even when CA is performed in specialized laboratories \cite{1979cook}. The lack of consensus and accessibility constraints limit practical deployment.

\subsection{Computer Vision as a Solution}

Machine learning (ML) and computer vision (CV) have revolutionized classification tasks across domains from medical imaging to autonomous vehicles. Within the realm of ML, Convolutional Neural Networks (CNNs) automatically extract hierarchical features from images, eliminating laborious manual feature engineering. Additionally, transfer learning leverages pre-trained CNNs; based on large datasets like ImageNet, these models enable high accuracy even with limited domain-specific data, avoiding a key obstacle in wildlife research.
...
The trail camera dataset comprises 197 color images collected from National Deer Association (NDA) products. While many of the images meet NDA quality standards (broadside head-up posture, minimal body movement, good lighting, etc.), screen captures taken from NDA videos and other media do not meet the same criteria, ultimately improving the variation of white-tailed buck images within the dataset. Due to their large readership base, images in the NDA dataset cover 14 states. Importantly, no metadata (image date, time, or location) are provided to the model, forcing predictions to rely solely on visual morphology. A small sample of the trail camera dataset is illustrated in Figure \ref{fig:trail_camera_sample}.

\begin{figure}[h! tbp]
\centering
\includegraphics[width=9cm]{images/Fig1_TrailCamSamples.jpg}
\caption{Human prediction accuracy for each age class are presented in the form of (left) a bar chart, and (right) a confusion matrix.}
...
Data augmentation was used to address the limited dataset size. The training set ($80\%$, $157$ images) was expanded $40$-fold through rotations ($\pm10°\deg$), horizontal flipping, brightness adjustments ($0.8-1.2\times$), contrast variation ($0.8-1.2\times$), and Gaussian noise addition ($\sigma=0.01$). Each transformation simulates natural field variation (ex. lighting changes throughout the day, deer orientation, and varying camera angles) without distorting body proportions. The test set ($40$ images) remained held out and non-augmented to provide unbiased evaluation.

\subsection{Model Architecture and Training}

Following a preliminary evaluation of traditional methods and $>60$ traditional ML architectures and alternative CNNs (EfficientNet \cite{2020tan}, DenseNet \cite{2018huang}), ResNet-18 \cite{2015he} was selected based on held-out test accuracy.

\begin{figure}[h! tbp]
\centering
\includegraphics[width=\textwidth]{images/Fig2_ResNet18Model.jpg}
\caption{Human prediction accuracy for each age class are presented in the form of (left) a bar chart, and (right) a confusion matrix.}
...
Following model development, inference utilizes Test-Time Augmentation (TTA), in which each model predicted on both the original image and its horizontal flip, using the average predictions used. The five models' TTA-averaged predictions were ensemble-averaged using softmax normalization to produce final classification probabilities.
