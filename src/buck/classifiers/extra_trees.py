...
# ----------------- USAGE EXAMPLE -----------------
def train_final_extra_trees(X_train, y_train, X_test, y_true, optimized_opts):
    """
    Train final Extra Trees with optimized parameters
    """
    print("🏆 Training Final Extra Trees...")

    final_classifier = ExtraTreesClassifier(
        random_state=optimized_opts["random_state"],
        n_estimators=optimized_opts["n_estimators"],
        max_depth=optimized_opts["max_depth"],
        criterion=optimized_opts["criterion"],
        class_weight=optimized_opts["class_weight"],
        min_samples_split=optimized_opts["min_samples_split"],
        min_samples_leaf=optimized_opts["min_samples_leaf"],
        min_weight_fraction_leaf=optimized_opts["min_weight_fraction_leaf"],
        max_features=optimized_opts["max_features"],
        max_leaf_nodes=optimized_opts["max_leaf_nodes"],
        min_impurity_decrease=optimized_opts["min_impurity_decrease"],
        ...
        ccp_alpha=optimized_opts["ccp_alpha"],
    ...
    print(f"✅ Final Extra Trees Results:")
    print(f"   Training Time: {train_time:.1f} seconds")
    print(f"   Final Accuracy: {final_accuracy:.4f}")
    print(f"   Final F1 Score: {final_f1:.4f}")
    print(f"   Number of Estimators: {final_classifier.n_estimators}")
    print(
        f"   Feature Importances Available: {len(final_classifier.feature_importances_)}"
    )

    print(f"\n   Optimized Parameters:")
    print(f"     Random State: {optimized_opts['random_state']}")
    print(f"     N_Estimators: {optimized_opts['n_estimators']}")
    print(f"     Criterion: {optimized_opts['criterion']}")
    print(f"     Max Depth: {optimized_opts['max_depth']}")
    print(f"     Max Features: {optimized_opts['max_features']}")
    print(f"     Bootstrap: {optimized_opts['bootstrap']}")
    print(f"     Class Weight: {optimized_opts['class_weight']}")
